...
if(openInv){
	...
	for(var k = 0; k < oPlayer.currentItemCount; k++){
		if(inventoryArray[k] != noone){
			...
			var current_item = inventoryArray[k];
			currentSprite = current_item.inventorySprite;
			
			//show_debug_message("drawing Item: " + string(current_item.name));
		...
		// type 2 - armor
			switch(current_item.type) {
				case 0:
					if(current_item.isEquipped){			 
						currentSprite = current_item.equippedSprite;
			...
			if (current_item.isEquipped == true) {
	...
	// Print the name of the item we are hovering over
	if (hoveredItemName != "") {
